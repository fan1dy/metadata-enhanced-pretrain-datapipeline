...
class MegatronDocumentTokenizer(PipelineStepWithTokenizer):
    ...
    def __init__(
        ...
        boc_token: str = "<|context_begin|>",  # Whether to add the BOC token before the context
        eoc_token: str = "<|context_end|>",  # Whether to add the EOC token after the context
        ...
        add_metadata_per_sequence: bool = False,
        mask_metadata: bool = True,
        metadata_builder = lambda doc: "",
        metadata_position: str = "left",
    ):
        ...
        self.add_metadata_per_sequence = add_metadata_per_sequence
        ...
        for batch in batched(data, self.batch_size):
            with self.track_time(unit="batch"):
                ...
                for document, encoded in zip(batch, encoded_batch):
                    tokens = encoded.ids
                    tokens_with_meta_data = []
                    padding_tokens = []
                    if self.add_metadata:
                        metadata = self.metadata_builder(document)
                        meta_tokens = self.tokenizer.encode(metadata,add_special_tokens=False).ids 
                        # assert len(meta_tokens) < self.sequence_length
                        # deal with edge case where meta data is longer than the sequence length
                        if len(meta_tokens) > self.sequence_length:
                            meta_tokens = self.tokenizer.encode(metadata[0:50],add_special_tokens=False).ids
                        meta_tokens = [self.tokenizer.token_to_id(self.boc_token)] + meta_tokens + [self.tokenizer.token_to_id(self.eoc_token)]
                        print("+++++++:",meta_tokens)
                        if self.offset_in_sequence + len(meta_tokens) >= self.sequence_length:
                            padding_tokens = [1] * (self.sequence_length - self.offset_in_sequence)
                            self.offset_in_sequence = 0
 
                        if self.metadata_position == "left":
                            # think how to handle the edge case when self.sequence_length - self.offset_in_sequence - len(meta_tokens), i.e., there is not enough tokens to put any tokens after the URL (or maybe even the URL does not fit in)
                            first_row = encoded.ids[0:self.sequence_length - self.offset_in_sequence - len(meta_tokens)]
                            first_row = [first_row[0]] + meta_tokens + first_row[1:] # this assumes that the first token is the special token
                        else:
                            first_row = encoded.ids[0:self.sequence_length - self.offset_in_sequence - len(meta_tokens)]
                            first_row = [first_row[0]]  + first_row[1:] + meta_tokens
                        tokens_with_meta_data += first_row
                        
                        if self.add_metadata_per_sequence:
                            for i in range(self.sequence_length - self.offset_in_sequence, len(tokens), self.sequence_length - len(meta_tokens)):
                                if self.metadata_position == "left":
                                    current_row = meta_tokens + encoded.ids[i: i+self.sequence_length - len(meta_tokens)]
                                else:
                                    current_row = encoded.ids[i: i+self.sequence_length - len(meta_tokens)] + meta_tokens
                                tokens_with_meta_data += current_row
                        else:
                            tokens_with_meta_data += encoded.ids[(self.sequence_length - self.offset_in_sequence):]

                        self.offset_in_sequence = (len(tokens_with_meta_data) + self.offset_in_sequence) % self.sequence_length

                    else:
                        tokens_with_meta_data = tokens
                                    
                    # Write bytes to disk
                    unshuff.write(padding_tokens + tokens_with_meta_data)
                    
                    # Save stats
                    self.stat_update("tokens", value=len(tokens_with_meta_data))
        unshuff.close()
        return unshuff
    ...
    def run(
        ...
        outputfile = self.write_tokens(data, prefix_filename)
        if len(outputfile) == 0:
